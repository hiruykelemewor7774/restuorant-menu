...
export default function StockInPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-200 text-gray-800 p-6 pt-10">
      ...
      <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-500 rounded-2xl p-6 max-w-lg space-y-3">
        ...
        <select value={form.supplierId} onChange={(e) => setForm({ ...form, supplierId: e.target.value })} className="w-full px-3 py-2 bg-gray-200 border border-gray-500 rounded-lg text-sm">
        ...
        <input type="number" placeholder="ጠቅላላ ዋጋ (Total Cost)" value={form.totalCost} onChange={(e) => setForm({ ...form, totalCost: e.target.value })} className="w-full px-3 py-2 bg-gray-200 border border-gray-500 rounded-lg text-sm" />
        <textarea placeholder="ማስታወሻ" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="w-full px-3 py-2 bg-gray-200 border text-gray-800 border-gray-500 rounded-lg text-sm" rows={2} />
